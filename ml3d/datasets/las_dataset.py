...
def read_pc(path, feature_dims):
    with laspy.open(path) as fh:
        las = fh.read()
        labels = np.array(las.classification,dtype=np.int32)
        points = np.vstack((las.x, las.y, las.z)).transpose()
        if len(feature_dims)>0:
            features = np.vstack([las[feat_dim] for feat_dim in feature_dims]).transpose()
        else:
            features = None 
        
    return points, features, labels

class LasDatasetSplit(BaseDatasetSplit):
    ...
    def __init__(self, dataset, split='training'):
        ...
        self.features_dims = self.cfg.features
    ...
    def get_data(self, idx):
        ...
        points, features, labels = read_pc(path, self.features_dims)
...
class LasDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.train_files = [f for f in glob.glob(self.train_dir + "/*.las")] + [f for f in glob.glob(self.train_dir + "/*.laz")]
        self.val_files = [f for f in glob.glob(self.val_dir + "/*.las")] + [f for f in glob.glob(self.val_dir + "/*.laz")]
        self.test_files = [f for f in glob.glob(self.test_dir + "/*.las")] + [f for f in glob.glob(self.test_dir + "/*.laz")]
    ...
    def get_split(self, split):
        ...
        Returns:
            A dataset split object providing the requested subset of the data.
        """
        return LasDatasetSplit(self, split=split)
    ...
    def save_test_result(self, results, attr):
        ...
        store_path = join(path, attr['name']+"_pred.laz")
        
        las.classification = results['predict_labels']
        las.write(store_path)


DATASET._register_module(LasDataset)
